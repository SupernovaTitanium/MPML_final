...
def bayesian_optimisation(n_iters, sample_loss, bounds, x0=None, n_pre_samples=5,
                          gp_params=None, random_search=False, alpha=1e-5, epsilon=1e-7, data=None, target=None):
    ...
    if x0 is None:
        for params in np.random.uniform(bounds[:, 0], bounds[:, 1], (n_pre_samples, bounds.shape[0])):
            ...
            y_list.append(sample_loss(params, data, target))
    else:
        for params in x0:
            x_list.append(params)
            y_list.append(sample_loss(params, data, target))
    ...
    for n in range(n_iters):
        ...
        cv_score = sample_loss(next_sample, data, target)
