...
def plot_iteration(filePath, first_param_grid, sampled_params, sampled_loss, first_iter=0, alpha=1e-5,
    ...
    for i in range(first_iter, sampled_params.shape[0] - 1):
        ...
        plt.savefig(filePath+'/bo_iteration_%d.png' % (i), bbox_inches='tight')
...
def _plot_loss_2d(first_param_grid, second_param_grid, sampled_params, sampled_loss, mu, ei, next_sample, optimum=None):
    ...
    ax2.set_title("Mean estimate of score surface for iteration %d" % (sampled_params.shape[0]))
    ax2.set_xlabel("C")
...
def show_true_loss_func(lambdas, gammas, real_loss, filePath):
    ...
    plt.colorbar(cp)
    plt.title('Filled contours plot of loss function for [gamma, C]')
    plt.xlabel('C')
    plt.ylabel('gamma')
    plt.savefig(filePath+'/real_score_contour.png', bbox_inches='tight')
    # plt.show()